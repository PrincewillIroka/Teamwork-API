...
const gifsRoutes = express.Router();
...
gifsRoutes.route("/").post(async function (request, response) {
  let status = {},
    { title } = request.body;
  ...
  if (title && token && file) {
    ...
    if (jwtVerification(token).hasError) {
      status = {
        status: "error",
        error: "Invalid token"
      };
    ...
    const userId = jwtVerification(token).userId;
